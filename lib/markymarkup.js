...
function renderBody (markup) {
  ...
  for (let line in allMatches(regexes.singleLineYAML, content)) {
    let html = nunjucks.render(line[1], { content: line[2] })
    content.replace(line[0], html)
  }

  // next render newline delimited blocks inside paragraphs
  // add newlines at top and bottom to tmake for easy matching
  content = '\n\n' + content + '\n\n'
  for (let graf in allMatches(regexes.bareGrafs, content)) {
    content.replace(graf[0], `<p>${graf[0]}</p>`)
  }
