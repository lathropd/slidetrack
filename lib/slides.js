...
var isSafari = /^((?!android).)*mobile*safari/i.test(navigator.userAgent);
...
let Slides = function (params) {
  let defaults = { 
    ...
    playButton: '#play',
    pauseButton: '#pause',
    skipForwardButton: '#skipForward',
    skipBackwardButton: '#skipBackward',
  ...
  this.currentSlide = 0
  this.player = WaveSurfer.create(WAVESURFER_SETTINGS)
  this.player.load(this.settings.audio)
  this.player.on('audioprocess', throttle(() => this.updateSlide(), 10))
  this.player.on('ready', () => {
    this.settings.animateIn(this.slides[this.currentSlide])
  })
  document.querySelector(this.settings.playButton).onclick = () => {
    this.player.play()
    this.updateSlide()
    return false
  }
  document.querySelector(this.settings.pauseButton).onclick = () => {
    this.player.pause()
    this.updateSlide()
    return false
  }
  document.querySelector(this.settings.skipForwardButton).onclick = () => {
    if (this.currentSlide < this.offsets.length - 1) {
      let nextTS = this.offsets[this.currentSlide + 1]
      this.player.seekTo(nextTS/this.player.getDuration() + 0.00001)
    } else {
      this.player.seekTo(1.0 - 0.00001)
    }
    this.updateSlide()
    return false
  }
  document.querySelector(this.settings.skipBackwardButton).onclick = () => {
    if (this.currentSlide > 0) {
      let prevTS = this.offsets[this.currentSlide - 1]
      this.player.seekTo(prevTS/this.player.getDuration() + 0.00001)
    } else {
      this.player.seekTo(0.00001)
    }
    this.updateSlide()
    return false
  }
